...
// Ta clé API CoinAPI ici :
const COINAPI_KEY = 'TA_CLE_COINAPI_ICI';

// Liste des cryptos à exclure
...
async function fetchCoinApiWeeklyClose(symbol) {
  try {
    ...
    if (!res.ok) throw new Error('CoinAPI pas dispo');
    const data = await res.json();
    const closes = data.map(o => o.close).filter(n => typeof n === 'number');
    if (closes.length > RSI_PERIOD) return closes;
  } catch (e) {
    console.warn(`CoinAPI fail pour ${symbol.toUpperCase()}:`, e.message);
  }
  return null;
}
