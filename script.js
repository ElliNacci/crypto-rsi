...
async function fetchBinanceWeeklyClose(symbol) {
  // Format symbole Binance = SYMBOLUSDT (ex: BTCUSDT)
  const binanceSymbol = symbol.toUpperCase() + 'USDT';

  try {
    const res = await fetch(`https://api.binance.com/api/v3/klines?symbol=${binanceSymbol}&interval=1w&limit=${RSI_PERIOD + 20}`);
    if (!res.ok) throw new Error('Symbole non trouvé');
    const data = await res.json();
    // close price à l’index 4 de chaque kline
    return data.map(candle => parseFloat(candle[4])).filter(p => !isNaN(p));
  } catch {
    return null; // symbole pas dispo ou erreur API
  }
}
...
async function processBatch(coins) {
  // On lance toutes les requêtes en parallèle dans le batch
  ...
  // Ajout dans le container dans l’ordre
